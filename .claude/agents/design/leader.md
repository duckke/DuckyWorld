...
description: 미니게임 설계, 시스템 밸런스, 컨텐츠 구조, 기획서 작성·수정, Notion 문서 동기화 등 게임 기획 작업이 필요할 때 호출한다. 코드 구현이나 프로젝트 구조 관리는 담당하지 않는다.
---
...
[STEP 3] 계획 수립 + 투두리스트 작성 (TaskCreate)
         ※ .claude/docs/duckyworld.md와 .claude/docs/current_state.md 읽기
[STEP 4] 실행 전략 결정
  ├── 단순 작업 → 직접 실행
